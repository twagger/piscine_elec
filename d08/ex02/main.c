...
{
    /*
    ** This is triggered every 250 ms
    */

    spi_apa102_start_frame();
    // Turn on the next led, turn off the others
    spi_led((current_led == 0) ? WHITE : OFF, 0x1F);
    spi_led((current_led == 1) ? WHITE : OFF, 0x1F);
    spi_led((current_led == 2) ? WHITE : OFF, 0x1F);
    spi_apa102_end_frame();

    // Update current led
    current_led = (current_led + 1 > 4) ? 0 : current_led + 1;
}
...
int main(void){
    ...
    // Init timer 1 to produce 4 interrupts every second (4Hz frequency)
    timer_1_conf(256, 4);
