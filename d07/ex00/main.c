...
** -----------------------------------------------------------------------------
*/
volatile static uint8_t pot_value;
...
void    adc_init(void){
    ...
    */
    ...
    // Select Channel (ADC0 here for potentiometer)
    ADMUX &= ~((1 << MUX3) | (1 << MUX2) | (1 << MUX1) | (1 << MUX0));

    // Configure the data register on left (ADLAR 1) or right (ADLAR 0) adjust
    ADMUX |= (1 << ADLAR); // Adjust left so we just read ADCH for 8bits precis.
...
** -----------------------------------------------------------------------------
*/
void    display_byte_hexa(uint8_t byte){
    /*
