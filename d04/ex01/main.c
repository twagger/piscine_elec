...
static const char *status_mess[] = {"A START condition has been transmitted", \
...
static const uint8_t status_codes[] = {TW_START, TW_MT_SLA_ACK, TW_MR_SLA_ACK, \
                                       TW_MT_DATA_ACK};
...
int check_for_status(uint8_t expected){
    ...
    */
    ...
    if ((TWSR & 0xF8) != expected){
        ...
        for (uint8_t i = 0; i < sizeof(status_codes) / sizeof(status_codes[0]); i++) {
